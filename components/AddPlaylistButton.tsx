...
const AddPlaylistButton = () => {
    const { createPlaylistOnSpotify, songs, initialPlaylistDescription } = useSiteContext();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [name, setPlaylistName] = useState(`"${initialPlaylistDescription}" Playlist`);
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        for (const song of songs)
            if (song.uri) songsIds.push(song.uri);
        const playlistData = {
            playlistData: {
                name: name,
    ...
    return (
        <>
            ...
            <div className="new-playlist-button">
                <button onClick={() => setIsModalOpen(true)}>Add Playlist to Spotify Account</button>
            </div>
